...
<c:choose>
    <c:when test="<%= themeDisplay.isSignedIn() %>">
        ...
        <c:if test="${authLFConfig.showSignOut}">
            <br/>
            <a href="/c/portal/logout" class="btn btn-primary auth-lf-signoutbutton"><liferay-ui:message key="sign-out"/></a>
        </c:if>
        <%-- /**  End modification by Triberay */ --%>
    </c:when>
    <c:otherwise>
        ...
        <%-- /**  Start modification by Triberay */ --%>
        <div class="authLFContainer  <c:out value="${authLFConfig.width}"/> <c:out value="${authLFConfig.align}"/> <c:out value="${authLFConfig.buttonStyle}"/>">
            <%@ include file="/navigation-social.jspf" %>
            <liferay-ui:panel-container accordion="true" extended="true" cssClass="login-panel-container email">
                <liferay-ui:panel defaultState="open" title="Email" iconCssClass="icon-envelope">
        ...
        <aui:form action="<%= loginURL %>" autocomplete='<%= PropsValues.COMPANY_SECURITY_LOGIN_FORM_AUTOCOMPLETE ? "on" : "off" %>' cssClass="sign-in-form" method="post" name="<%= formName %>" onSubmit="event.preventDefault();">
            ...
            <aui:fieldset>
                ...
                <aui:input autoFocus="<%= windowState.equals(LiferayWindowState.EXCLUSIVE) || windowState.equals(WindowState.MAXIMIZED) %>" cssClass="clearable" label="<%= loginLabel %>" name="login" showRequiredLabel="<%= false %>" type="text" value="<%= login %>" placeholder="<%= loginLabel %>">
        ...
        <%-- /**  Start modification by Triberay */ --%>
            ...
            <%@ include file="/navigation-email.jspf" %>
        </div>
        <%-- /**  End modification */ --%>
